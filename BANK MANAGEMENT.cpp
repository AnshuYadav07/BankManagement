...
//declaring gotoxy == function for cursor position
...
//structure for amount transfer track
...
void account(void){
	...
	printf("\n\n PASSWORD: ");										//password in form of stars
	for(i=0;i<50;i++){
		ch=getch();
		if(ch!=13){
			password[i]=ch;
...
//display function to show show the data of the user on screen
...
void display(char username1[]){
	...
	while(fread(&u1,sizeof(u1),1,fp)){
		if(strcmp(username1,u1.username)==0){
			...
			gotoxy(55,18);
			printf("PHONE NUMBER: %s",u1.pnumber);
	...
	gotoxy(0,6);                //menu to perform different functions by user
	printf("  HOME  ");
...
void transfermoney(void){
	...
	//checking for username if it is present or not
	...
	display(usernamet);              //func to return to home screen
}
...
void checkbalance(char username2[]){
	...
	//reading username to fetch correct data
